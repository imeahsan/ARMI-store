...
const AuctionModal = ({
  ...
  const handleMoreInfo = (slug) => {
    ...
    handleLogEvent("auction", `opened ${slug} product details`);
  };

  const category_name = showingTranslateValue(product?.category?.name)
    ?.toLowerCase()
    ?.replace(/[^A-Z0-9]+/gi, "-");
  console.log("auctions", product, "stock", stock);
      ...
      <MainModal modalOpen={modalOpen} setModalOpen={setModalOpen}>
        <div className="inline-block overflow-y-auto h-full align-middle transition-all transform bg-white shadow-xl rounded-2xl">
          <div className="flex flex-col lg:flex-row md:flex-row w-full max-w-4xl overflow-hidden">
            <Link href={`/auctions/${product.slug}`} passHref>
              <div
            ...
            <div className="w-full flex flex-col p-5 md:p-8 text-left">
              <div className="mb-2 md:mb-2.5 block -mt-1.5">
                <div onClick={() => handleMoreInfo(product.slug)}>
                  <h1
                    onClick={() => setModalOpen(false)}
                ...
                >
                  <Bids bids={product?.bids?.length} modal />
                </div>
              </div>
