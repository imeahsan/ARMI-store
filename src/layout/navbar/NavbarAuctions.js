...
const NavbarAuctions = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // return;
    if (searchText) {
      router.replace(`/auctions?query=${searchText}`, null, { scroll: false });
      // setSearchText("");
            ...
            <div className="hidden md:hidden md:items-center lg:flex xl:block absolute inset-y-0 right-0 pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
              ...
              >
                {imageUrl || userInfo?.image ? (
                  ...
                  <Link
                    href="/user/dashboard"
                    className="leading-none font-bold font-serif block text-black"
                  >
                    {userInfo?.name[0]}
                  </Link>
                ) : (
...
export default dynamic(() => Promise.resolve(NavbarAuctions), { ssr: false });
